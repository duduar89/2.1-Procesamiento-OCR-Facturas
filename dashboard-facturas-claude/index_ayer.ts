...
async function ejecutarCotejoAutomatico(documentoId) {
  try {
...
async function getAuthenticatedUserContext(req, supabaseClient) {
  ...
  try {
    ...
    const { data: userProfile, error: profileError } = await supabaseClient.from('usuarios').select('restaurante_id').eq('id', user.id).single();
    ...
    const { data: restaurante, error: restauranteError } = await supabaseClient.from('restaurantes').select('id, nombre, cif, direccion, ciudad').eq('id', restauranteId).single();
...
async function smartRestaurantFilter(fullText, restauranteContext, supabaseClient) {
  ...
  const foundCIFs = [
    ...new Set(fullText.match(cifPattern) || [])
  ];
...
function extractCoordinates(field, confidence, fieldValue) {
  try {
...
function extractCoordinatesFromField(fieldValue) {
  try {
...
function extractFromFormFields(formFields, fullText, extractedData) {
...
function extractFromEntities(entities, fullText, extractedData) {
  ...
  entities.forEach((entity, index)=>{
    try {
      ...
      if (normalizedType.includes('proveedor') || normalizedType.includes('empresa') || normalizedType.includes('compañia') || normalizedType.includes('compañía') || normalizedType.includes('entidad') || normalizedType.includes('supplier') || normalizedType.includes('vendor') || normalizedType.includes('company') || normalizedType.includes('seller') || normalizedType.includes('merchant')) {
      ...
      if (normalizedType.includes('cif') || normalizedType.includes('nif') || normalizedType.includes('identificación') || normalizedType.includes('identificacion') || normalizedType.includes('tax') || normalizedType.includes('vat') || normalizedType.includes('ein') || normalizedType.includes('business') || normalizedType.includes('registration')) {
      ...
      if (normalizedType.includes('factura') || normalizedType.includes('numero') || normalizedType.includes('número') || normalizedType.includes('documento') || normalizedType.includes('folio') || normalizedType.includes('referencia') || normalizedType.includes('invoice') || normalizedType.includes('document') || normalizedType.includes('reference') || normalizedType.includes('number')) {
        // Verificar que no sea una fecha u otro número
        if (normalizedValue.trim().length >= 1 && !normalizedValue.includes('/') && !normalizedValue.includes('-') && !normalizedValue.includes('.') && !/^\d{1,2}[\/\-\.]\d{1,2}[\/\-\.]\d{2,4}$/.test(normalizedValue)) {
      ...
      if (normalizedType.includes('fecha') || normalizedType.includes('emisión') || normalizedType.includes('emision') || normalizedType.includes('date') || normalizedType.includes('time')) {
      ...
      if (normalizedType.includes('dinero') || normalizedType.includes('importe') || normalizedType.includes('total') || normalizedType.includes('precio') || normalizedType.includes('coste') || normalizedType.includes('costo') || normalizedType.includes('valor') || normalizedType.includes('money') || normalizedType.includes('currency') || normalizedType.includes('amount') || normalizedType.includes('price') || normalizedType.includes('cost')) {
      ...
      if (normalizedType.includes('dirección') || normalizedType.includes('direccion') || normalizedType.includes('domicilio') || normalizedType.includes('ubicación') || normalizedType.includes('ubicacion') || normalizedType.includes('address') || normalizedType.includes('location')) {
  ...
  extractedData.confianza_global = Math.max(0.3, (extractedData.confianza_proveedor + extractedData.confianza_datos_fiscales + extractedData.confianza_importes) / 3);
...
function extractDataFromFormParser(googleAIResponse) {
  ...
  try {
    const document1 = googleAIResponse.document;
    if (!document1) {
      throw new Error('No se encontró documento en la respuesta');
    }
    console.log('📄 Documento Form Parser encontrado en respuesta');
    console.log('📝 Texto presente:', !!document1.text);
    console.log('📝 Longitud texto:', document1.text?.length || 0);
    console.log('📄 Páginas presentes:', !!document1.pages);
    console.log('📄 Número de páginas:', document1.pages?.length || 0);
    // 🔍 VERIFICAR ESTRUCTURA DE PÁGINAS
    if (document1.pages && document1.pages.length > 0) {
      document1.pages.forEach((page, pageIndex)=>{
    ...
    console.log('🔍 DEBUGGING: document.pages existe?', !!document1.pages);
    console.log('🔍 DEBUGGING: document.pages.length:', document1.pages?.length || 0);
    if (document1.pages && document1.pages.length > 0) {
      document1.pages.forEach((page, pageIndex)=>{
        ...
        if (page.formFields && page.formFields.length > 0) {
          ...
          try {
            extractFromFormFields(page.formFields, document1.text, extractedData);
    ...
    if (document1.tables && document1.tables.length > 0) {
      console.log(`📊 ${document1.tables.length} tablas encontradas a nivel de documento`);
    ...
    if (document1.entities && document1.entities.length > 0) {
      console.log(`🔍 ${document1.entities.length} entities encontradas a nivel de documento`);
      extractFromEntities(document1.entities, document1.text, extractedData);
    }
    ...
    if (camposCriticosFaltantes.length > 0) {
      ...
      const fallbackData = extractDataFromTextFallback(document1.text);
    ...
    const datosFallback = extractDataFromTextFallback(document?.text || '');
...
function extractDataFromGoogleAI(googleAIResponse) {
  console.log('🌍 Iniciando extracción desde Google AI OCR...');
  try {
    const document1 = googleAIResponse.document;
    if (!document1 || !document1.text) {
    ...
    if (document1.pages && document1.pages.length > 0) {
      ...
      const getTextFromAnchor = (fullText, textAnchor)=>{
        try {
        ...
        containers.forEach((container)=>{
          if (container.items && container.items.length > 0) {
            ...
            container.items.forEach((item, idx)=>{
              const itemText = getTextFromAnchor(document1.text, item.layout?.textAnchor);
    ...
    const datosExtraidos = extractDataFromTextFallback(document1.text);
    ...
    if (Object.keys(coordenadasCampos).length === 0) {
      datosExtraidos.campos_con_baja_confianza = [
...
function extractDataFromTextFallback(text) {
  ...
  function extractWithPatterns(patterns, text) {
    const results = [];
  ...
  function extractWithPattern(pattern, text) {
    return [
      ...text.matchAll(pattern)
  ...
  const matches = {
    ...
    tipoIva: patterns.tipoIva.flatMap((pattern)=>extractWithPattern(pattern, text)).map((t)=>parseInt(t)).filter((n)=>!isNaN(n) && n >= 0 && n <= 30)
  };
    ...
    for(let i = 0; i < lines.length; i++){
      if (lines[i].includes('INSCRITA EN EL REGISTRO MERCANTIL')) {
        ...
        for(let j = i + 1; j <= Math.min(i + 3, lines.length - 1); j++){
          const candidateLine = lines[j].trim();
          if (candidateLine.length > 5 && candidateLine.length < 120 && !candidateLine.includes('CIF') && !candidateLine.includes('TELÉFONO') && !candidateLine.includes('C/') && !candidateLine.includes('POLÍGONO') && !candidateLine.includes('INDUSTRIAL') && !candidateLine.includes('CTRA') && !candidateLine.includes('KM') && !candidateLine.includes('CÁDIZ') && !candidateLine.includes('SANLÚCAR') && !candidateLine.includes('BARRAMEDA')) {
            proveedorNombre = candidateLine;
  ...
  if (matches.numeroFactura.length > 0) {
    const candidatos = matches.numeroFactura.map((f)=>f.replace(/\s+/g, ' ').trim()).filter((f)=>f.length >= 1 && f.length <= 25).filter((f)=>/\d/.test(f)) // Debe tener al menos un número
    ;
  ...
  let tipoIva = matches.tipoIva.find((t)=>[
      4,
      10,
      21
    ].includes(t)) || matches.tipoIva[0] || 21;
      ...
      segments.forEach((segment, index)=>{
        if (segment && typeof segment === 'object') {
          ...
          if (startIndex !== undefined && endIndex !== undefined) {
            ...
            if (!isNaN(start) && !isNaN(end) && start >= 0 && end <= fullText.length && start < end) {
...
function extractProductsFromFullText(fullText) {
...
function extractProductsFromFormParser(extractedResult) {
  ...
  try {
    ...
    if (extractedResult.document && extractedResult.document.pages && extractedResult.document.pages.length > 0) {
      extractedResult.document.pages.forEach((page, pageIndex)=>{
        if (page.tables && page.tables.length > 0) {
          ...
          page.tables.forEach((table, tableIndex)=>{
            ...
            if (table.headerRows && table.headerRows.length > 0) {
              const headers = table.headerRows[0].cells.map((cell)=>getTextFromAnchor(extractedResult.document.text || '', cell.layout.textAnchor));
              console.log(`📋 Headers de tabla:`, headers);
              // Buscar columnas relevantes
              const cantidadIndex = headers.findIndex((h)=>/cantidad|uds?|unidades?|qty/i.test(h));
              const descripcionIndex = headers.findIndex((h)=>/descripci[oó]n|concepto|art[ií]culo|producto|item/i.test(h));
              const precioIndex = headers.findIndex((h)=>/precio|importe|total|euros?|€/i.test(h));
              const codigoIndex = headers.findIndex((h)=>/c[oó]digo|ref|referencia|sku/i.test(h));
              ...
              if (table.bodyRows && table.bodyRows.length > 0) {
                table.bodyRows.forEach((row, rowIndex)=>{
                  try {
                    const cells = row.cells.map((cell)=>getTextFromAnchor(extractedResult.document.text || '', cell.layout.textAnchor));
                    ...
                    if (descripcion && descripcion.length > 2 && precio && precio > 0) {
                      ...
                      console.log(`❌ Fila descartada - datos insuficientes:`, {
                        descripcion,
                        precio
                      });
                    }
          ...
          if (page.formFields && page.formFields.length > 0) {
            page.formFields.forEach((field)=>{
              ...
              if (fieldName && fieldValue && fieldValue.trim().length > 3) {
                const isProductField = /producto|art[ií]culo|concepto|descripci[oó]n|item|mercancia|mercancía/i.test(fieldName) || /carne|meat|kg|unidad|gramos|litros|pieza/i.test(fieldValue) || fieldValue.length > 10 && /\d/.test(fieldValue) && /[a-záéíóúñ]/i.test(fieldValue);
                if (isProductField) {
                  const producto = {
...
async function searchProveedorByName(nombreProveedor, restauranteId, supabaseClient) {
  ...
  try {
    ...
    if (existingProveedores && existingProveedores.length > 0) {
      ...
      const mejorCoincidencia = existingProveedores.find((prov)=>prov.nombre.toLowerCase().includes(nombreNormalizado.split(' ')[0])) || existingProveedores[0];
      console.log(`✅ Proveedor encontrado por nombre: ${mejorCoincidencia.nombre} (${mejorCoincidencia.id})`);
      // Actualizar último contacto
      await supabaseClient.from('proveedores').update({
...
async function processProveedorUpsert(nombreProveedor, cifProveedor, restauranteId, supabaseClient) {
  ...
  try {
    ...
    const { data: restauranteData, error: restauranteError } = await supabaseClient.from('restaurantes').select('cif, nombre').eq('id', restauranteId).single();
    ...
    const { data: otrosRestaurantes, error: otrosError } = await supabaseClient.from('restaurantes').select('cif, nombre').eq('cif', cifProveedor).neq('id', restauranteId);
    ...
    const { data: existingProveedor, error: searchError } = await supabaseClient.from('proveedores').select('id, nombre, numero_facturas, total_facturado').eq('restaurante_id', restauranteId).eq('cif', cifProveedor).single();
    ...
    if (existingProveedor && !searchError) {
      ...
      const { error: updateError } = await supabaseClient.from('proveedores').update({
        fecha_ultima_factura: new Date().toISOString().split('T')[0],
...
async function findExistingProduct(descripcionOriginal, restauranteId, supabaseClient) {
  ...
  let { data: exactMatch, error: exactError } = await supabaseClient.from('productos_maestro').select('id, precio_ultimo, nombre_normalizado, nombre_comercial').eq('restaurante_id', restauranteId).eq('nombre_comercial', descripcionOriginal).single();
  ...
  let { data: normalizedMatch, error: normalizedError } = await supabaseClient.from('productos_maestro').select('id, precio_ultimo, nombre_normalizado, nombre_comercial').eq('restaurante_id', restauranteId).eq('nombre_normalizado', normalizado).single();
  ...
  const palabrasClave = descripcionOriginal.toLowerCase().replace(/[^a-zA-Z0-9\sñáéíóúü]/g, ' ') // Limpiar caracteres especiales
  .split(' ').filter((word)=>word.length > 2) // Solo palabras de 3+ caracteres
  .filter((word)=>![
      'del',
      'con',
      'sin',
  ...
  if (palabrasClave.length > 0) {
    ...
    let queryBuilder = supabaseClient.from('productos_maestro').select('id, precio_ultimo, nombre_normalizado, nombre_comercial').eq('restaurante_id', restauranteId);
...
function calculateProductSimilarity(original, comercial, normalizado) {
...
function calculateStringSimilarity(str1, str2) {
  ...
  for (const word1 of words1){
    for (const word2 of words2){
      ...
      if (word1.length > 3 && word2.length > 3 && (word1.includes(word2) || word2.includes(word1))) {
        matches += 0.8;
        break;
      }
...
async function processProductsUpsert(productos, restauranteId, proveedorId, documentId, supabaseClient, proveedorNombre, numeroFactura, fechaFactura// ✅ NUEVO: Fecha de la factura
  ...
  for (const producto of productos){
    try {
      ...
      const existingProduct = await findExistingProduct(producto.descripcion_original, restauranteId, supabaseClient);
      let productoMaestroId;
      if (existingProduct) {
        ...
        await updateProductPriceStatistics(existingProduct.id, producto.precio_unitario_sin_iva, producto.peso_neto, producto.formato_comercial, supabaseClient);
        ...
        const { data: newProduct, error: insertError } = await supabaseClient.from('productos_maestro').insert(nuevoProductoMaestro).select('id').single();
        if (insertError) {
      ...
      if (productoMaestroId && producto.precio_unitario_sin_iva > 0) {
        ...
        await createPriceHistoryEntry(productoEnriquecido, productoMaestroId, restauranteId, proveedorId, documentId, supabaseClient, fechaFactura // ✅ NUEVO: Pasar fecha de la factura
        );
      }
...
function inferCategory(description) {
  ...
  if (desc.includes('queso') || desc.includes('leche') || desc.includes('yogur') || desc.includes('mantequilla') || desc.includes('nata') || desc.includes('crema') || desc.includes('stilton') || desc.includes('gouda') || desc.includes('brie') || desc.includes('alp blossom') || desc.includes('tres leches')) {
    return 'lacteos';
  }
  // 🥩 CÁRNICOS
  if (desc.includes('carne') || desc.includes('pollo') || desc.includes('cerdo') || desc.includes('ternera') || desc.includes('cordero') || desc.includes('jamón') || desc.includes('embutido') || desc.includes('pastrami') || desc.includes('lomo')) {
    return 'carnes';
  }
  // 🐟 PESCADOS
  if (desc.includes('pescado') || desc.includes('merluza') || desc.includes('salmon') || desc.includes('gambas') || desc.includes('marisco') || desc.includes('anchoa') || desc.includes('boquerón') || desc.includes('gildas')) {
    return 'pescados';
  }
  // 🥬 VERDURAS
  if (desc.includes('verdura') || desc.includes('tomate') || desc.includes('lechuga') || desc.includes('gordal') || desc.includes('patatas') || desc.includes('patata')) {
    return 'verduras';
  }
  // 🍎 FRUTAS
  if (desc.includes('fruta') || desc.includes('manzana') || desc.includes('naranja') || desc.includes('almendra') || desc.includes('almendras')) {
    return 'frutas';
  }
  // 🍞 PANADERÍA
  ...
  // 🍺 BEBIDAS
  if (desc.includes('cerveza') || desc.includes('vino') || desc.includes('refresco') || desc.includes('gin') || desc.includes('ron') || desc.includes('ginebra') || desc.includes('heineken') || desc.includes('cruzcampo') || desc.includes('beefeater') || desc.includes('seagram') || desc.includes('puerto de indias') || desc.includes('barcelo')) {
    return 'bebidas';
  }
  // 🧹 LIMPIEZA
  if (desc.includes('limpieza') || desc.includes('detergente') || desc.includes('papel') || desc.includes('lejía') || desc.includes('jabón') || desc.includes('limpiador')) {
    return 'limpieza';
  }
  // 🚚 ENVÍOS Y LOGÍSTICA
  if (desc.includes('envio') || desc.includes('envío') || desc.includes('portes') || desc.includes('frio') || desc.includes('frío') || desc.includes('barril') || desc.includes('caja') || desc.includes('cubitos') || desc.includes('hielo')) {
    return 'logistica';
  }
  // 🏷️ DESCUENTOS Y SERVICIOS
  if (desc.includes('descuento') || desc.includes('mantenimiento') || desc.includes('gestión') || desc.includes('redes sociales') || desc.includes('merrychef')) {
    return 'servicios';
  }
  // 🧂 CONDIMENTOS
  if (desc.includes('aceite') || desc.includes('vinagre') || desc.includes('sal') || desc.includes('pimienta') || desc.includes('especia') || desc.includes('condimento')) {
    return 'condimentos';
  }
  return 'general';
}
...
function extractProductFormat(description) {
...
function inferFormatFromDescription(description) {
  ...
  const patronesUnidades = [
    {
      pattern: /(\d+)\s*(?:uds?|unidades?)/i,
      tipo: 'unidad'
    },
    {
      pattern: /(\d+)\s*(?:pcs?|piezas?)/i,
      tipo: 'unidad'
    },
    {
      pattern: /(\d+)\s*(?:cajas?)/i,
...
function validateFormatRange(formato, peso, volumen, description) {
  ...
  if (peso !== null) {
    ...
    if (peso > 50) {
      return {
        valido: false,
        razon: 'Peso demasiado alto para producto individual'
      };
    }
    if (peso < 0.001) {
      return {
        valido: false,
        razon: 'Peso demasiado bajo'
      };
    }
    ...
    if (description.includes('pollo') && (peso < 0.3 || peso > 5)) {
      return {
        valido: false,
        razon: 'Peso de pollo fuera de rango típico'
      };
    }
    if (description.includes('huevo') && peso > 2) {
      return {
        valido: false,
        razon: 'Peso de huevos sospechoso'
      };
    }
    if (description.includes('pan') && (peso < 0.1 || peso > 2)) {
      return {
        valido: false,
        razon: 'Peso de pan fuera de rango típico'
      };
    }
  }
  // Validar volumen
  if (volumen !== null) {
    if (volumen > 20) {
      return {
        valido: false,
        razon: 'Volumen demasiado alto para producto individual'
      };
    }
    if (volumen < 0.01) {
      return {
        valido: false,
        razon: 'Volumen demasiado bajo'
      };
    }
    // Validaciones específicas por categoría
    if (description.includes('cerveza') && volumen > 2) {
      return {
        valido: false,
        razon: 'Volumen de cerveza sospechoso'
      };
    }
  }
  return {
    valido: true,
    razon: 'Formato válido'
  };
}
// 💰 FUNCIÓN MEJORADA PARA CALCULAR PRECIO POR KG
function calculatePricePerKg(precioUnitario, pesoNeto, descripcion) {
...
function calculatePricePerLiter(precioUnitario, volumen, descripcion) {
...
function fixPriceCalculation(producto) {
  ...
  if (producto.precio_unitario_con_iva && (!producto.precio_unitario_sin_iva || producto.precio_unitario_sin_iva === 0)) {
  ...
  if (producto.precio_unitario_sin_iva && (!producto.precio_unitario_con_iva || producto.precio_unitario_con_iva === 0)) {
  ...
  if (producto.precio_total_linea_con_iva && (!producto.precio_total_linea_sin_iva || producto.precio_total_linea_sin_iva === 0)) {
  ...
  if (producto.precio_total_linea_sin_iva && (!producto.precio_total_linea_con_iva || producto.precio_total_linea_con_iva === 0)) {
  ...
  if ((!producto.precio_unitario_sin_iva || producto.precio_unitario_sin_iva === 0) && producto.precio_total_linea_sin_iva && producto.cantidad && producto.cantidad > 0) {
    producto.precio_unitario_sin_iva = producto.precio_total_linea_sin_iva / producto.cantidad;
    producto.precio_unitario_con_iva = producto.precio_unitario_sin_iva * factorIva;
  ...
  if ((!producto.precio_total_linea_sin_iva || producto.precio_total_linea_sin_iva === 0) && producto.precio_unitario_sin_iva && producto.cantidad && producto.cantidad > 0) {
  ...
  if (producto.precio_unitario_con_iva && producto.cantidad && (!producto.precio_total_linea_sin_iva || producto.precio_total_linea_sin_iva === 0)) {
    producto.precio_unitario_sin_iva = producto.precio_unitario_con_iva / factorIva;
    producto.precio_total_linea_sin_iva = producto.precio_unitario_sin_iva * producto.cantidad;
  ...
  if (producto.precio_total_linea_con_iva && producto.cantidad && (!producto.precio_unitario_sin_iva || producto.precio_unitario_sin_iva === 0)) {
    producto.precio_total_linea_sin_iva = producto.precio_total_linea_con_iva / factorIva;
    producto.precio_unitario_sin_iva = producto.precio_total_linea_sin_iva / producto.cantidad;
    producto.precio_unitario_con_iva = producto.precio_unitario_sin_iva * factorIva;
...
async function createPriceHistoryEntry(producto, productoMaestroId, restauranteId, proveedorId, documentId, supabaseClient, fechaFactura) {
  ...
  try {
    const historialEntry = {
      ...
      // METADATOS MEJORADOS
      fecha_compra: fechaFactura || new Date().toISOString().split('T')[0],
      fecha_registro: new Date().toISOString(),
      tipo_documento: 'factura',
      numero_documento: producto.numero_factura || null,
    ...
    const { data: historialResult, error: historialError } = await supabaseClient.from('historial_precios_productos').insert(historialEntry).select();
    if (historialError) {
...
async function updateProductPriceStatistics(productoMaestroId, nuevoPrecio, pesoUnitarioKg, contenidoNeto, supabaseClient) {
  console.log('📈 === ACTUALIZANDO ESTADÍSTICAS DE PRECIOS ===');
  try {
    // 1. Obtener producto maestro actual para comparar
    const { data: productoActual, error: productoError } = await supabaseClient.from('productos_maestro').select('precio_ultimo, precio_minimo_historico, precio_maximo_historico, numero_compras_historicas').eq('id', productoMaestroId).single();
    if (productoError) {
      console.error('❌ Error obteniendo producto actual:', productoError);
      return;
    }
    // 2. Obtener historial de precios de los últimos 30 días
    const { data: historialPrecios, error: historialError } = await supabaseClient.from('historial_precios_productos').select('precio_unitario_sin_iva, fecha_compra, documento_id, proveedor_nombre').eq('producto_maestro_id', productoMaestroId).gte('fecha_compra', new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]).order('fecha_compra', {
      ascending: false
    });
    if (historialError) {
    ...
    if (productoActual.precio_ultimo && productoActual.precio_ultimo > 0) {
      ...
      if (historialPrecios && historialPrecios.length > 0) {
        const precioAnteriorEntry = historialPrecios.find((h)=>h.precio_unitario_sin_iva === productoActual.precio_ultimo);
    ...
    const { error: updateError } = await supabaseClient.from('productos_maestro').update({
...
function mapCoordinatesToExtractedData(openaiData, coordenadasOCR) {
  console.log('🗺️ === MAPEANDO COORDENADAS ===');
  const coordenadasCampos = {};
  // Helper para encontrar coordenadas por texto
  function findCoordinatesByText(textoFuente) {
...
function convertOpenAIToExpectedFormat(openaiData, coordenadasCampos) {
  ...
  return {
    ...
    confianza_global: ((factura.proveedor_nombre?.confianza || 0) + (factura.total_factura?.confianza || 0) + (factura.numero_factura?.confianza || 0)) / 3,
    confianza_proveedor: factura.proveedor_nombre?.confianza || 0,
    confianza_datos_fiscales: ((factura.numero_factura?.confianza || 0) + (factura.fecha_factura?.confianza || 0) + (factura.proveedor_cif?.confianza || 0)) / 3,
    confianza_importes: ((factura.total_factura?.confianza || 0) + (factura.base_imponible?.confianza || 0) + (factura.cuota_iva?.confianza || 0)) / 3,
    coordenadas_campos: coordenadasCampos,
    campos_con_baja_confianza: []
  };
}
// 🛒 FUNCIÓN PARA PROCESAR PRODUCTOS DE OpenAI
function processOpenAIProducts(openaiData) {
  ...
  return productos.map((producto, index)=>{
    ...
    if (productoCorregido.precio_unitario_sin_iva > 0) {
      productoCorregido.precio_por_kg = calculatePricePerKg(productoCorregido.precio_unitario_sin_iva, formato.peso_neto, productoCorregido.descripcion_original // 🆕 Añadir descripción
      );
      productoCorregido.precio_por_litro = calculatePricePerLiter(productoCorregido.precio_unitario_sin_iva, formato.volumen, productoCorregido.descripcion_original // 🆕 Añadir descripción
      );
    }
...
function extractCoordinatesFromOCR(extractedResult) {
  console.log('📍 === EXTRAYENDO COORDENADAS DEL OCR ===');
  const coordenadasCampos = {};
  try {
    const document1 = extractedResult.document;
    if (!document1 || !document1.pages || document1.pages.length === 0) {
    ...
    document1.pages.forEach((page, pageIndex)=>{
      ...
      const containers = [
        ...
        {
          key: 'tableRow',
          items: page.tableRows || [],
          conf: 0.8
        },
        {
          key: 'tableCell',
          items: page.tableCells || [],
          conf: 0.8
        }
      ];
      containers.forEach((container)=>{
        if (container.items && container.items.length > 0) {
          ...
          container.items.forEach((item, idx)=>{
            ...
            if (item.layout?.textAnchor) {
              itemText = getTextFromAnchor(document1.text, item.layout.textAnchor);
            } else if (item.fieldName?.textAnchor) {
              itemText = getTextFromAnchor(document1.text, item.fieldName.textAnchor);
            } else if (item.fieldValue?.textAnchor) {
              itemText = getTextFromAnchor(document1.text, item.fieldValue.textAnchor);
            } else if (item.headerRows && item.headerRows.length > 0) {
              itemText = 'Tabla: ' + item.headerRows.map((row)=>row.cells.map((cell)=>getTextFromAnchor(document1.text, cell.layout?.textAnchor)).join(' | ')).join('; ');
            } else if (item.bodyRows && item.bodyRows.length > 0) {
              itemText = 'Filas: ' + item.bodyRows.map((row)=>row.cells.map((cell)=>getTextFromAnchor(document1.text, cell.layout?.textAnchor)).join(' | ')).join('; ');
            }
            const coords = extractCoordinates(item, container.conf, itemText);
            if (coords) {
              const key = `pagina_${pageIndex + 1}_${container.key}_${idx + 1}`;
...
async function notifyReviewNeeded(telefono, classification, nombreArchivo) {
  try {
  ...
  try {
    ...
    try {
      ...
      if (authHeader && authHeader === `Bearer ${serviceRoleKey}`) {
        ...
        const { data: documentInfo, error: docError } = await supabaseClient.from('documentos').select('restaurante_id, restaurantes!inner(id, nombre, cif, direccion, ciudad)').eq('id', documentId).single();
    ...
    const { data: documentInfo, error: docError } = await supabaseClient.from('documentos').select('url_storage, nombre_archivo, restaurante_id').eq('id', documentIdFinal).single();
    if (docError || !documentInfo) {
    ...
    await supabaseClient.from('documentos').update({
      estado: 'processing',
    ...
    const { data: fileData, error: downloadError } = await supabaseClient.storage.from('documentos').download(filePath);
    if (downloadError) {
    ...
    if (extractedResult.document) {
      ...
      if (extractedResult.document.pages) {
        ...
        extractedResult.document.pages.forEach((page, index)=>{
          ...
          if (page.formFields && page.formFields.length > 0) {
            ...
            page.formFields.forEach((field, fieldIndex)=>{
    ...
    const { error: classificationUpdateError } = await supabaseClient.from('documentos').update(updateData).eq('id', documentIdFinal);
    ...
    if (tipoConfirmado) {
      ...
      if (documentClassification.confianza < CONFIDENCE_THRESHOLD) {
        ...
        const { error: revisionUpdateError } = await supabaseClient.from('documentos').update({
    ...
    const contextAnalysis = await smartRestaurantFilter(fullText, userContext.restaurante, supabaseClient);
    ...
    console.log('📋 Tipos de coordenadas encontradas:', [
      ...new Set(Object.values(coordenadasOCR).map((c)=>c.tipo))
    ]);
    ...
    const { data: restauranteCheck, error: checkError } = await supabaseClient.from('restaurantes').select('cif, nombre').eq('id', documentInfo.restaurante_id).single();
...
`;
    ...
    const proveedorResult = await processProveedorUpsert(extractedData.proveedor_nombre || 'Sin nombre', extractedData.proveedor_cif || 'Sin CIF', documentInfo.restaurante_id, supabaseClient);
    ...
    if (proveedorResult && extractedData.proveedor_nombre) {
      try {
        ...
        if (proveedorEmbedding.length > 0) {
          await saveProveedorEmbedding(supabaseClient, proveedorResult.id, documentInfo.restaurante_id, extractedData.proveedor_nombre, proveedorEmbedding);
    ...
    const productosConMaestroId = await processProductsUpsert(productosExtraidos, documentInfo.restaurante_id, proveedorResult?.id || null, documentIdFinal, supabaseClient, extractedData.proveedor_nombre, extractedData.numero_factura, extractedData.fecha_factura);
    ...
    const { data: tableInfoFacturas, error: tableErrorFacturas } = await supabaseClient.from('datos_extraidos_facturas').select('*').limit(1);
    ...
    const { data: tableInfoAlbaranes, error: tableErrorAlbaranes } = await supabaseClient.from('datos_extraidos_albaranes').select('*').limit(1);
    ...
    if (documentClassification.tipo === 'factura') {
      ...
      datosParaInsertar = {
        ...datosParaInsertar,
        numero_albaran: extractedData.numero_factura,
        fecha_albaran: extractedData.fecha_factura,
        total_albaran: extractedData.total_factura,
        base_imponible: extractedData.base_imponible,
        cuota_iva: extractedData.cuota_iva,
        tipo_iva: extractedData.tipo_iva,
    ...
    const { data: insertResult, error: insertError } = await supabaseClient.from(tablaDestino).insert(datosParaInsertar).select();
    if (insertError) {
    ...
    if (productosConMaestroId.length > 0) {
      ...
      try {
        ...
        const { data: productosInsertResult, error: productosInsertError } = await supabaseClient.from('productos_extraidos').insert(productosParaInsertar).select();
        if (productosInsertError) {
          ...
          try {
            const { data: historialVerificacion, error: historialError } = await supabaseClient.from('historial_precios_productos').select('id, producto_maestro_id, documento_id, precio_unitario_sin_iva, fecha_compra').eq('documento_id', documentIdFinal).order('fecha_compra', {
              ascending: false
            });
            if (historialError) {
    ...
    const { data: updateResult, error: updateError } = await supabaseClient.from('documentos').update({
      estado: estadoFinal,
      estado_cotejacion: estadoCotejacion,
      url_storage: storageUrl // Guardar URL completa en lugar del path relativo
    }).eq('id', documentIdFinal).select();
    if (updateError) {
...
✅ El ${tipoDocumento.toLowerCase()} ya está disponible en tu sistema.
 Puedes revisar los productos y precios extraídos.`;
    ...
    try {
      ...
      if (documentClassification?.tipo && [
        'factura',
        'albaran'
      ].includes(documentClassification.tipo)) {
        ...
        if (resultadoCotejo.success) {
          ...
          if (documentClassification.tipo === 'albaran' && resultadoCotejo.enlaces_automaticos > 0) {
            ...
            const { error: updateError } = await supabase.from('datos_extraidos_albaranes').update({
    ...
    return new Response(JSON.stringify({
      ...
      needsReview: documentClassification?.confianza < 0.8,
      estadoCotejacion: estadoCotejacion,
...
async function generateProductEmbedding(descripcion) {
...
function detectProductCategory(descripcion) {
...
async function saveProductEmbedding(supabaseClient, productoId, restauranteId, descripcion, embedding) {
  ...
  try {
    ...
    const { data: existing, error: checkError } = await supabaseClient.from('productos_embeddings').select('id').eq('restaurante_id', restauranteId).eq('descripcion_original', descripcion).single();
    ...
    if (existing) {
      ...
      const { error: updateError } = await supabaseClient.from('productos_embeddings').update({
        embedding: embedding,
        categoria: categoria,
        frecuencia_uso: 1,
      ...
      // Insertar nuevo
      const { error: insertError } = await supabaseClient.from('productos_embeddings').insert({
        producto_id: null,
        restaurante_id: restauranteId,
...
async function processProductEmbeddings(supabaseClient, productos, restauranteId) {
  ...
  for (const producto of productos){
    try {
      ...
      if (embedding.length > 0) {
        ...
        // Guardar embedding
        await saveProductEmbedding(supabaseClient, producto.id, restauranteId, producto.descripcion_original, embedding);
        processedCount++;
...
async function saveProveedorEmbedding(supabaseClient, proveedorId, restauranteId, nombreProveedor, embedding) {
  if (embedding.length === 0) {
  ...
  try {
    // Verificar si ya existe
    const { data: existing, error: checkError } = await supabaseClient.from('proveedores_embeddings').select('id').eq('restaurante_id', restauranteId).eq('nombre_proveedor', nombreProveedor).single();
    if (checkError && checkError.code !== 'PGRST116') {
    ...
    if (existing) {
      // Actualizar existente
      const { error: updateError } = await supabaseClient.from('proveedores_embeddings').update({
        embedding: embedding,
        frecuencia_uso: 1,
        ultima_actualizacion: new Date().toISOString()
      }).eq('id', existing.id);
      ...
      // Insertar nuevo
      const { error: insertError } = await supabaseClient.from('proveedores_embeddings').insert({
        proveedor_id: proveedorId,
        restaurante_id: restauranteId,
